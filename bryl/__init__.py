...
class Context(threading.local):
    ...
    def __init__(self, **defaults):
        self.stack = [
            self._Frame(
                alpha_filter=False, alpha_truncate=False, alpha_upper=False, **defaults
            )
        ]
...
class Field(object):
    ...
    def __init__(
        ...
        self._order = next(self._order) if order is None else order
        self.name = name
        self.length = length
    ...
    def map(self, record, value):  # noqa: C901
        if value is not None:
            ...
            if error:
                raise self.error_type(
                    "Invalid {0}.{1} value {2} for - {3}".format(
                        type(record).__name__, self.name, value, error
                    )
                )
            return value
    ...
    def pack(self, value):
        ...
        if self.align == self.LEFT:
            value = value + str(self.pad) * (self.length - len(value))
        elif self.align == self.RIGHT:
            value = (str(self.pad) * (self.length - len(value))) + value
        else:
...
class Datetime(Field):
    ...
    format_re = re.compile(
        "Y{4}|Y{2}|D{3}|D{2}|M{2}|" "h{2}|H{2}|m{2}|s{2}|X{2}|Z{3}|p{2}"  # day  # time
    )
    ...
    def _insert_tz(cls, raw, tz, spec):
        offset, length = spec
        if len(tz) != length:
            raise ValueError('Timezone "{0}" length != {1}'.format(tz, length))
    ...
    def load(self, raw):
        ...
        if self._tz:
            ...
            if tz not in self.time_zones:
                raise ValueError(
                    'Unsupported time-zone "{0}", expected one of {1}'.format(
                        tz, self.time_zones.keys()
                    )
                )
...
class Date(Datetime):
    ...
    format_re = re.compile("Y{4}|Y{2}|D{3}|D{2}|M{2}|J{3}")  # day
...
class Time(Datetime):
    ...
    format_re = re.compile("h{2}|H{2}|m{2}|s{2}|X{2}|Z{3}|p{2}")  # time
...
class RecordMeta(type):
    def __new__(mcs, name, bases, dikt):
        ...
        for field in cls.fields:
            for base in bases:
                if not hasattr(base, field.name) or not isinstance(
                    getattr(base, field.name), Field
                ):
                    continue
...
class LineReader(Reader):
    ...
    def next_record(self, expected_type=None, default="raise"):
        ...
        try:
            ...
            raise
        except self.record_type.field_type.error_type as ex:
            self.retry = line, line_no
    ...
    def next(self):
        ...
        if (
            self.expected_terminal is not None
            and record_terminal != self.expected_terminal
        ):
            self.malformed(line_no, 'unexpected EOL "{0}"'.format(record_terminal))
